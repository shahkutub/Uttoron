...
lateinit var destURLImg: String
...
class DownloadImageFileFromURLTask(val mContext: Context, private val outputDir: String, private val srcURL: String,private val fileName: String, private val downloadListener: DownloadListener) : AsyncTask<String?, Int?, Boolean>() {
    ...
    init {
        ...
        progressDialog!!.setTitle("Downloading "+"Image/Icon")
    }
    ...
    override fun doInBackground(vararg f_url: String?): Boolean {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
                ...
                destURLImg = Environment.getExternalStorageDirectory().path + File.separator + outputDir
                var desFile = File(destURLImg)
                if (!desFile.exists()) {
                    desFile.mkdir()
                }
//                else{
...
//                }
                ...
                destURLImg = destURLImg + File.separator + Environment.DIRECTORY_DOWNLOADS
                desFile = File(destURLImg)
                if (!desFile.exists()) {
                    desFile.mkdir()
                }

                destURLImg = destURLImg + File.separator + fileName

                // final output path
                outputPath = destURLImg
                ...
                Log.i(TAG, "Downloading destination url: $destURLImg")
                Log.i(TAG, "Downloading -> Below Android 11")
                outputStream = FileOutputStream(destURLImg)
